package HPC::Runner::App::Scheduler;
...
with 'HPC::Runner::App::Scheduler::ParseInput';

use HPC::Runner::App::Scheduler::JobStats;
use HPC::Runner::App::Scheduler::JobDeps;

=head1 HPC::Runner::App::Scheduler
...
has 'job_stats' => (
    is      => 'rw',
    isa     => 'HPC::Runner::App::Scheduler::JobStats',
    default => sub {
        return HPC::Runner::App::Scheduler::JobStats->new();
    }
);
...
sub check_add_to_jobs {
    ...
    if ( !exists $self->jobs->{ $self->jobname } ) {
        $self->jobs->{ $self->jobname }
            = HPC::Runner::App::Scheduler::JobDeps->new();
    }
